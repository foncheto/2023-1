...
def find_paths(matrix, row, col, visited, path, paths, zeros_last_column):
    # Verificar límites de la matriz y si la celda ya fue visitada
    if (
        row < 0 or row >= len(matrix)
    ...
    find_paths(matrix, row - 1, col, visited, path, paths, zeros_last_column)  # vecino superior
    find_paths(matrix, row + 1, col, visited, path, paths, zeros_last_column)  # vecino inferior
    find_paths(matrix, row, col - 1, visited, path, paths, zeros_last_column)  # vecino izquierdo
    find_paths(matrix, row, col + 1, visited, path, paths, zeros_last_column)  # vecino derecho
...
paths_with_indices = enumerate(paths, 1)
paths_formatted = map(
    lambda path: (
        f"Camino {path[0]}:\n"
        + "\n".join([f"({row}, {col})" for row, col in path[1]])
        + "\n"
    ),
    paths_with_indices,
)

print(*paths_formatted, sep="\n")